...
struct PlayView: View {
    ...
    // Live playback-speed control (§5.3) — available for Practice and Listen
    // (Demo); Play stays full-speed since it's scored. Updates the engine
    // immediately so a Demo loop already in progress re-tracks the new tempo.
    private var tempoControl: some View {
        HStack(spacing: 6) {
            ForEach([0.5, 0.75, 1.0], id: \.self) { scale in
                Button {
                    ...
                    Text("\(Int(scale * 100))%")
                        ...
                        .padding(.vertical, 6)
                        .background(
                            app.tempoScale == scale ? Theme.accent : Color.white.opacity(0.15),
                            in: Capsule()
                        )
                        .foregroundStyle(app.tempoScale == scale ? .black : .white)
                }
                .buttonStyle(.plain)
            }
        }
