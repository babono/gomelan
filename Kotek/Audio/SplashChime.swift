...
final class SplashChime {
    ...
    private static func buildAudibleStroke() -> Data? {
        ...
        return encodeWAV(mixed, sampleRate: Int(source.format.sampleRate))
    }

    /// Mono 16-bit PCM in a minimal RIFF container.
    ///
    /// `AVAudioPlayer(data:)` wants a container it recognises, not bare samples.
    /// Going back out through a WAV keeps the playback path identical to the one
    /// already proven to work here, which after several silent attempts is worth
